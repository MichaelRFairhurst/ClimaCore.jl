...
function rhs!(dY, Y, _, t)
    ...
    ᶜw = @. Ic(ρw / Yfρ)
    ᶠw = @. ρw / Yfρ
    u⃗ = @. Geometry.UWVector(ᶜuₕ) + Geometry.UWVector(ᶜw)
    # Horizontal Momentum Contribution
    ᶜ∂u∂x = @. hgrad(ᶜuₕ)
    ᶠ∂u∂z = @. If(Geometry.transform(Geometry.WAxis(),
                                     ∂c(If(ρuₕ/ρ))))
    ᶠ∂w∂z = @. If(Geometry.transform(Geometry.UAxis(),
                                     ∂c(ᶠw)))

    # Vertical Momentum Contribution
    ᶜ∂w∂z = @. Geometry.transform(Geometry.WAxis(),
                                     ∂c(ᶠw))
    ᶠ∂w∂x = @. hgrad(ᶠw)
    ᶜ∂u∂z = @. Geometry.transform(Geometry.WAxis(),
                                     ∂c(uₕf))
    ### APPLY SMAGORINSKY
    κₛ = 1.0 # m^2/s
    #  1a) horizontal div of horizontal grad of horiz momentun
    @. dρuₕ += hwdiv(κₛ * (ρ * ᶜ∂u∂x))
    #  1b) vertical div of vertical grad of horiz momentun
    @. dρuₕ += uvdivf2c(κₛ * (Yfρ * ᶠ∂u∂z))
    ᶠ∂w∂z₁ = @. ᶠ∂w∂z.components.data.:1
    @. dρuₕ += ∂c(κₛ * (Yfρ * ᶠ∂w∂z₁))

    #  1c) horizontal div of horizontal grad of vert momentum
    @. dρw += hwdiv(κₛ * (Yfρ * ᶠ∂w∂x))
    #  1d) vertical div of vertical grad of vert momentun
    @. dρw += vvdivc2f(κₛ * (ρ * ᶜ∂w∂z))
    ᶜ∂u∂z = @. Geometry.WVector(ᶜ∂u∂z.components.data.:1)
    #@. dρw += vvdivc2f(κₛ * (ρ * ᶜ∂u∂z₁))

    #  2a) horizontal div of horizontal grad of potential temperature
    @. dρθ += hwdiv(κₛ * (ρ * hgrad(ρθ / ρ)))
