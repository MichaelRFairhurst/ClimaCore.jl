...
function DistributedTopology2D(
    ...
    ghost_neighbor_elem_pairs = Tuple{Int, Int}[]
    ...
    for (lidx, gidx) in enumerate(local_elem_gidx)
        ...
        for gidx in temp_ghost_neighbor_elemset
            push!(ghost_neighbor_elem_pairs, (gidx, lidx))
        end
    end
    ...
    for i in eachindex(ghost_neighbor_elem_pairs)
        (gidx, lidx) = ghost_neighbor_elem_pairs[i]
        ridx = global_elem_ridx[gidx]
        ghost_neighbor_elem_pairs[i] = (ridx, lidx)
    end
    sort!(ghost_neighbor_elem_pairs)
    # compute ragged storage
    ghost_neighbor_elem = Int[]
    ghost_neighbor_elem_offset = Int[]
    prev_ridx = 0
    for (i, (ridx, lidx)) in enumerate(ghost_neighbor_elem_pairs)
        if ridx != prev_ridx
            push!(ghost_neighbor_elem_offset, i)
            prev_ridx = ridx
        end
        push!(ghost_neighbor_elem, lidx)
    end
    push!(ghost_neighbor_elem_offset, length(ghost_neighbor_elem_pairs) + 1)
